...
function Home() {
  ...
  useEffect(() => {
    ...
    // Find the full building object from BUILDINGS list (to get images, floors etc.)
    const fullBuilding = BUILDINGS.find(
      (b) => String(b.id) === String(state.building!.id),
    );
    ...
    // Start navigation to this building immediately
    setRoute({ start: null, dest: destBuilding });
    setCurrentStepIndex(0);

    // Show the building modal too so the user sees where they're headed
  ...
  /* ─── UI ───────────────────────────────────────── */
  return (
    <div className="h-screen w-full relative overflow-hidden bg-gradient-to-br from-blue-100 via-white to-purple-100">
      ...
      {/* ✅ Navigation started banner (from Buildings page) */}
      {route.dest && path.length > 0 && (
        <div
          style={{
            ...
            fontSize: 13,
            fontWeight: 600,
            boxShadow: "0 4px 16px rgba(47,70,147,0.4)",
            display: "flex",
            alignItems: "center",
            gap: 8,
            whiteSpace: "nowrap",
        ...
        >
          🧭 Navigating to {route.dest.name.split(" ").slice(0, 3).join(" ")}
          <button
      ...
      <div className="absolute inset-0 pt-16 flex items-center justify-center">
        <div className="relative w-full max-w-5xl h-[85%] bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-200 transition duration-300 hover:shadow-[0_20px_60px_rgba(0,0,0,0.2)]">
      ...
      {/* 📍 ROUTE PANEL */}
      {path.length > 0 && (
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 w-full max-w-3xl z-50 px-4">
